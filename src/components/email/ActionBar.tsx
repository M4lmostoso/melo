...
export function ActionBar({ thread, messages, noReply, defaultReplyMode = "reply", contactSidebarVisible, taskSidebarVisible, onReply, onReplyAll, onForward, onPrint, onExport, onPopOut, onToggleContactSidebar, onToggleTaskSidebar }: ActionBarProps) {
  ...
  return (
    <>
      <div data-tauri-drag-region className="action-bar flex items-center gap-1 px-3 py-3 border-b border-border-secondary bg-bg-secondary">
        {/* Reply / Forward group */}
        {hasLastMessage && (
          <>
            <Button
        ...
        <Button variant="secondary" iconOnly icon={<Download size={15} strokeWidth={1.5} />} onClick={onExport} title={t("actionBar.exportEml")} />
        <Button variant="secondary" iconOnly icon={<ExternalLink size={15} strokeWidth={1.5} />} onClick={onPopOut} title={t("actionBar.openInNewWindow")} />
        <Button
          variant="secondary"
          iconOnly
