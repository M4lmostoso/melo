...
function buildGroups(tasks: DbTaskWithSubject[]): ThreadGroup[] {
  ...
  // Overdue groups first, then by nearest due date, general section last
  groups.sort((a, b) => {
...
export function TasksPage() {
  ...
  const handleRestoreAll = useCallback(async () => {
    for (const task of filteredDeletedTasks) await restoreTask(task.id);
    await loadDeletedTasks();
    await loadTasks();
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border-primary shrink-0 bg-bg-primary/60 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          >
            ...
            <option value="deleted">{t("tasks.page.trash")}</option>
          </select>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            {isTrash ? (
              <>
                ...
                <p className="text-sm text-text-secondary mb-1">{t("tasks.page.noDeletedTasks")}</p>
              </>
            ) : (
              <>
