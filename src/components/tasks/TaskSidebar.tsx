    ...
    <div className="w-72 border-l border-border-primary bg-bg-primary/50 flex flex-col shrink-0">
      ...
      <div className="flex flex-col border-b border-border-secondary">
        <div className="flex items-center justify-between px-4 py-3">
          <h3 className="text-sm font-semibold text-text-primary">{t("tasks.sidebar.title")}</h3>
          <div className="flex items-center gap-1">
            {/* AI Monitor toggle */}
            <button
              onClick={handleToggleMonitor}
              title={isMonitored ? t("tasks.sidebar.aiMonitoringActive") : t("tasks.sidebar.enableAiExtraction")}
            ...
            >
              ...
              <span className="hidden sm:inline">
                {isMonitored ? t("tasks.sidebar.aiOn") : t("tasks.sidebar.ai")}
              </span>
            </button>
            ...
            {threadTasks.length > 0 && (
              <button
                ...
                title={t("tasks.sidebar.deleteAllTasksTitle")}
            ...
            <button
              ...
              title={t("tasks.sidebar.openTasksPage")}
        ...
        {isMonitored && !isDraftLoading && (() => {
          ...
          const hasOverdue = threadTasks.some(
            (task) => !task.is_completed && task.due_date !== null && task.due_date < now,
          );
          return (
            <div className={`flex items-center gap-1.5 px-4 pb-2 ${hasOverdue ? "text-red-400" : "text-accent"}`}>
              ...
              <span className="text-[0.625rem] font-medium">
                {hasOverdue ? t("tasks.sidebar.overdueMonitoring") : t("tasks.sidebar.monitoringActive")}
              </span>
            </div>
          );
        })()}
        ...
        <div className="border-b border-accent/20 bg-accent/5 px-4 py-3 space-y-2">
          ...
          {isDraftLoading && (
            <div className="flex items-center gap-2 text-xs text-text-secondary py-1">
              ...
              {t("tasks.sidebar.analyzingThread")}
            </div>
          )}
              ...
              <div key={i} className="rounded-lg border border-border-primary bg-bg-primary/60 px-3 py-2 space-y-1.5">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className={`inline-flex items-center gap-0.5 text-[0.625rem] px-1.5 py-0.5 rounded ${dir.color}`}>
                    {dir.icon}
                    {draft.direction === "incoming" ? t("tasks.sidebar.incoming") : t("tasks.page.outbox")}
                  </span>
                  {draft.priority !== "none" && (
                  ...
                  {draft.dueDate && (
                    <span className="text-[0.625rem] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-tertiary">
                      {new Date(draft.dueDate * 1000).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                    </span>
                  )}
                </div>
                {draft.description && (
      ...
      {deletedCount > 0 && (
        <div className="flex items-center justify-between px-4 py-2 border-b border-amber-500/20 bg-amber-500/5">
          <span className="text-xs text-amber-400">
            {deletedCount === 1
              ? t("tasks.sidebar.deletedBanner", { count: deletedCount })
              : t("tasks.sidebar.deletedBannerPlural", { count: deletedCount })}
          </span>
          <button
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {threadTasks.length === 0 ? (
          <p className="text-xs text-text-tertiary text-center py-6">
            {t("tasks.sidebar.noTasksForThread")}
          </p>
        ) : (
      ...
      <div className="border-t border-border-secondary">
        <TaskQuickAdd onAdd={handleAddTask} placeholder={t("tasks.sidebar.addTaskToThread")} />
      </div>
    </div>
  );
}
